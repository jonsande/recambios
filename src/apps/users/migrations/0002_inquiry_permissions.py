...
INQUIRY_MODELS = ("Inquiry", "InquiryItem")
...
def add_inquiry_permissions_to_internal_staff(apps, schema_editor):
    Group = apps.get_model("auth", "Group")
    group, _ = Group.objects.get_or_create(name=ROLE_INTERNAL_STAFF)

    permission_ids = []
    for model_name in INQUIRY_MODELS:
...
def remove_inquiry_permissions_from_internal_staff(apps, schema_editor):
    Group = apps.get_model("auth", "Group")
    Permission = apps.get_model("auth", "Permission")
    ...
    codenames = [f"{action}_{model_name.lower()}" for model_name in INQUIRY_MODELS for action in ALL_ACTIONS]
    permissions = Permission.objects.filter(content_type__app_label="inquiries", codename__in=codenames)
...
class Migration(migrations.Migration):
    dependencies = [
        ...
        ("users", "0001_role_groups"),
    ]

    operations = [
        migrations.RunPython(
            add_inquiry_permissions_to_internal_staff,
            remove_inquiry_permissions_from_internal_staff,
        ),
    ]
